...
export async function extractParameters(state: GraphState): Promise<Partial<GraphState>> {
  ...
  const prompt = ChatPromptTemplate.fromMessages([
    ["system", `You are an expert software engineer. You're provided with a list of required and optional parameters for an API, along with a users query.
...
["human", `Query: {query}`]
  ...
  const schema = z.object({
    params: z.record(z.string()).describe("The parameters extracted from the query.").optional(),
  ...
  const requiredParams = bestApi?.required_parameters.map((p) => `Name: ${p.name}, Description: ${p.description}, Type: ${p.type}`).join("\n");
  const optionalParams = bestApi?.optional_parameters.map((p) => `Name: ${p.name}, Description: ${p.description}, Type: ${p.type}`).join("\n");
